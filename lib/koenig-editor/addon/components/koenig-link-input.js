...
export default Component.extend({
    ...
    drop(event) {
        ...
        if (event.dataTransfer.files) {
            ...
            const documentFile = event.dataTransfer.files[0];
            const uploadUrl = this.get('ghostPaths.url').api('documents', 'upload');

            formData.append('file', documentFile, documentFile.name);
            
            this.ajax.post(uploadUrl, {
                data: formData,
                processData: false,
                contentType: false,
                dataType: 'text'
            }).then((response) => {
                let [document] = get(JSON.parse(response), 'documents');
                let documentUrl = document.url;
                let href = relativeToAbsolute(documentUrl, this.config.get('blogUrl'));
                this.set('href', href);
                this.editor.run((postEditor) => {
                    this._replaceLink(href, postEditor);
                });

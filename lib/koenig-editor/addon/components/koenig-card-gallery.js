...
export default Component.extend({
    ...
    imageRows: computed('images.@each.{src,previewSrc,width,height,row}', function () {
        ...
        let maxImagesInRow = function (idx) {
            return noOfImages > 1 && (noOfImages % MAX_PER_ROW === 1) && (idx === (noOfImages - MAX_PER_ROW + 1));
        };
        ...
        this.images.forEach((image, idx) => {
            ...
            } else if (((idx + 1) % MAX_PER_ROW === 0) || maxImagesInRow(idx + 1) || idx + 1 === noOfImages) {
                // last image in row
                classes.push('pl2');
    ...
    _dragStart(draggableInfo) {
        ...
        if (isImageDrag && draggableInfo.payload.src) {
            this._dragDropContainer.enableDrag();
        }
    },

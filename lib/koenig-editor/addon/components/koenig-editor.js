...
// helper function to insert media cards at or after the current active section
// used when pasting or dropping media files
function insertMediaCards(files, postEditor, mediaType) {
    let {builder, editor} = postEditor;
    let collection = editor.post.sections;
    let section = editor.activeSection;

    if (!editor.isDraft && mediaType === 'video') {
        return;
    }

    // when dropping an media on the editor before it's had focus there will be
    // no active section so we insert the media at the end of the document
    if (!section) {
        section = editor.post.sections.tail;

        // create a blank paragraph at the end of the document if needed because
        // we use `insertSectionBefore` and don't want the media to be added
    ...
    // the whole list ready to be replaced by the media cards
    ...
    // insert an media card for each media, keep track of the last card to be
    // inserted so that the cursor can be placed on it at the end
    let lastMediaSection;
    files.forEach((file) => {
        let payload = {
            files: [file]
        };
        lastMediaSection = builder.createCardSection(mediaType, payload);
        postEditor.insertSectionBefore(collection, lastMediaSection, section);
    });
    ...
    // place cursor on the last inserted media
    postEditor.setRange(lastMediaSection.tailPosition());
}
...
function countCharacters(text) {
    ...
    text = text.replace(/\s+/g, ''); // strip whitespaces
...
export default Component.extend({
    ...
    willRender() {
        ...
        if (this.editor) {
            this.editor.isDraft = this.isDraft;
        }
        ...
        run.schedule('afterRender', this, this._calculateCharacterCount);
    },
    ...
    postDidChange(editor) {
        ...
        // re-calculate character count
        this._calculateCharacterCount();
    ...
    handlePaste(event) {
        ...
        // if we have media files pasted, create an media card for each and set
        // the payload.files property which will cause the media to be auto-uploaded
        // NOTE: browser support varies as of May 2018:
        // - Safari: will paste all medias
        // - Chrome: will only paste the first media
        // - Firefox: will not paste any medias
        ...
        if (images.length > 0) {
            ...
            editor.run((postEditor) => {
                insertMediaCards(images, postEditor, 'image');
            });
            return;
        }
        ...
        if (videos.length > 0) {
            ...
            editor.run((postEditor) => {
                insertMediaCards(videos, postEditor, 'video');
            });
            return;
        }
    ...
    handleDrop(event) {
        ...
        if (event.dataTransfer.files) {
            ...
            /*
            let documents = Array.from(event.dataTransfer.files).filter(file => file.type.indexOf('pdf') > -1);
    ...
    _calculateCharacterCount() {
        run.throttle(this, this._throttledCharacterCount, 100, false);
    },

    _throttledCharacterCount() {
        if (this.isDestroying || this.isDestroyed) {
            return;
        }
        ...
        this.editor.post.walkAllLeafSections((section) => {
            if (!section.isCardSection) {
                characterCount += countCharacters(section.text);
            }
        });
        ...
        if (characterCount !== this.characterCount) {
            ...
            this.characterCountDidChange({characterCount});
        }
    },

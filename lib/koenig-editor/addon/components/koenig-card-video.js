...
export default Component.extend({
    ...
    init() {
        ...
        if (!this.payload) {
            ...
            if (!this.payload.posters && this.payload.poster) {
                this.payload.posters = [this.payload.poster];
            }
    ...
    actions: {
            ...
            this.ajax.request(`${processingUrl}${pid.url}/`).then((task) => {
                ...
                if (task.status === 'finished') {
                    this.editor.run(() => {
                        ...
                        this._updatePayloadAttr('posters', [task.data.poster]);
        ...
        takeSnapshot(event) {
            ...
            const posterUrl = this.get('ghostPaths.url').api('videos/poster');
            const videoElement = this.$('video').get(0);
            const videoUrl = encodeURIComponent(videoElement.src);
            const videoTime = videoElement.currentTime;

            this.set('takingSnapshot', true);

            this.ajax.request(`${posterUrl}?src=${videoUrl}&time=${videoTime}`).then((result) => {
                if (result.error) {
                    throw result.error;
                }
                this.set('takingSnapshot', false);
                this._updatePayloadAttr('posters', [...this.payload.posters, result.src]);
            }).catch((error) => {
                this.notifications.showAPIError(error);
            });
        },
        ...
        selectPoster(event) {
            ...
            const posterUrl = $(target)
                .closest('.poster')
                .find('img')
                .get(0).src;

            this._updatePayloadAttr('poster', posterUrl);
        }
    },
